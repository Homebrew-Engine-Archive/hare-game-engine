...
{
    char *retval = __PHYSFS_platformCopyEnvironmentVariable("USER");
    return(retval);
...
{
    //FIXME: PSP seems do not have fsync ?
    int fd = *((int *) opaque);
    FILE* fp = fdopen(fd, "rw");
    if (fp) fflush(fp);
    BAIL_IF_MACRO(fp == 0, strerror(errno), 0);
    return(1);
